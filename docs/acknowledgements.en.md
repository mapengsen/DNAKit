...
DNAKit's wheel and sdist do not include DiProDB numerical tables, Primer3, NUPACK, DSSR/3DNA, Dashing, BLAST, MMseqs2, sourmash, REBASE, JASPAR, DNA base model checkpoints or their databases, nor do they automatically download external programs. When the CLI adapter is present, the user must still install it separately and provide an explicit path. The model checkpoint is only downloaded to the local `ckpt/` when the user explicitly calls the representation API, and is not equivalent to being redistributed with DNAKit.

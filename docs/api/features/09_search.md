...
搜索结果使用 0-based 半开区间坐标；motif/PWM、功能位点和重复序列等规则扫描见[序列模式搜索](06_patterns.md)。
...
**限制：** 比较的是完整字面序列；空查询只匹配空目标，受 `max_targets` 和 `max_matches` 限制。
...
**限制：** 不跨环状原点；IUPAC 是字面匹配；空查询会匹配每个边界。
...
**限制：** query 不可为空；query 和 target 都必须是线性且不含 Gap；动态规划受 `max_cells` 限制。
...
**限制：** 使用字面子串搜索；回文 query 默认合并为 `both` 链命中。
...
**限制：** 当前是内存 sketch 的确定性全扫描，不是 ANN 或 BLAST/MMseqs2 数据库搜索。
...
**限制：** 文件带 SHA-256 自校验且默认拒绝覆盖；当前不创建 BLAST、MMseqs2 或 sourmash 外部索引。

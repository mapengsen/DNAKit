# `site/index.html` 实际功能分支树

> 提取范围：`site/index.html` 中“快速入门教程”的实际渲染导航，以及导航所链接生成页面中的实际功能。提供方、总览、示例、限制、边界、字段表和实测记录等说明文字不作为功能树节点；树叶细化到 `1. CORE-001 DNA序列对象` 这一层。
...
│       ├── 6. DATA-017 · 染色体划分
│       ├── 7. DATA-018 · 个体划分
│       ├── 8. 按自定义 label 划分（donor、family、locus 等）
│       └── 9. DATA-023 · 泄漏检测
├── 3. 数据分析
│   ├── （1）DNA描述符+表征+指纹
│   │   ├── DNA描述符
...
│   │   │   ├── 1. FP-001 整数编码
│   │   │   ├── 2. FP-002 One-hot编码
│   │   │   ├── 3. FP-003 k-mer 特征（含 FP-004 Canonical 模式）
...
│   └── （3）序列距离与比对
│       ├── 1. 序列距离
│       │   ├── SIM-006 · Hamming distance
│       │   └── SIM-007 · Edit distance
│       └── 2. 成对序列比对（SIM-008）
│           ├── 全局比对
│           ├── 局部比对
│           └── 半全局比对
